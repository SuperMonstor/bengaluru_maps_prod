...
export async function getMaps(page = 1, limit = 10): Promise<MapsResult> {
  try {
    ...
    const { data, error: mapsError, count } = await supabase
      .from("maps")
      .select(
        `
        id,
    ...
    console.log("Raw maps data (unauthenticated check):", JSON.stringify(mapsData, null, 2));
    if (!mapsData.length) {
    ...
    const [locationCountsRes, voteCountsRes, contributorCountsRes] = await Promise.all([
    ...
    const locationCounts = new Map<string, number>(
      locationCountsRes.data?.map((l: { map_id: string; location_count: number }) => [
        l.map_id,
    ...
    const contributorCounts = new Map<string, number>(
      contributorCountsRes.data?.map((c: { map_id: string; contributor_count: number }) => [
        c.map_id,
    ...
    const maps: MapResponse[] = mapsData.map((map) => {
      ...
      return {
        ...
        username: user ? `${user.first_name || "Unnamed"} ${user.last_name || "User"}`.trim() : "Unknown User",
  ...
  try {
    ...
    return {
      data: {
        ...
        locations: data.locations || [], // Return the full locations array, not the length
...
export async function createLocation({
  ...
  try {
    ...
    if (location.startsWith("http")) {
      ...
      try {
        ...
        if (pathParts.includes("place") && url.hash) {
          ...
          const coordsMatch = hashParams.get("q") || url.hash.match(/@(-?\d+\.\d+),(-?\d+\.\d+)/);
          if (coordsMatch) {
            const coords = coordsMatch instanceof Array ? coordsMatch.slice(1) : coordsMatch.split(",");
              ...
              placesService.findPlaceFromQuery(
                ...
                (results, status) => {
                  if (status === google.maps.places.PlacesServiceStatus.OK && results && results[0]?.geometry?.location) {
                    ...
                    googleMapsUrl = `https://www.google.com/maps/place/${encodeURIComponent(name.replace(/ /g, "+"))}/@${latitude},${longitude},15z`;
                  }
                  resolve(null);
                }
              );
        ...
        throw new Error("Invalid Google Maps URL format. Please ensure it’s a valid place URL.");
      }
    } else {
        ...
        placesService.findPlaceFromQuery(
          ...
          (results, status) => {
            if (status === google.maps.places.PlacesServiceStatus.OK && results && results[0]?.geometry?.location) {
              ...
              throw new Error("Could not find location. Please check the location name or URL.");
            }
            resolve(null);
          }
        );
    ...
    if (isNaN(latitude) || isNaN(longitude) || latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180) {
      throw new Error("Invalid latitude or longitude values derived from the location.");
    }
