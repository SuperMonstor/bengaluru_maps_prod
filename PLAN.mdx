...
### Phase 3: Refactor Map Logic
- [ ] Refactor `lib/hooks/useGoogleMaps.ts`:
  - Rename to `useMapLogic.ts` (optional, or just keep and strip).
  - **Comment out** `fetchPlaceDetails` logic.
  - Remove `useLoadScript` if we are fully removing the Google Maps dependency for the *view* page (we might still need it for the *submit* page if we keep autocomplete there, but for the view page it should be gone).
...
- **Refactor vs Rewrite**:
  ...
  - `useGoogleMaps.ts`: **Refactor** (Commenting out code).
